...
<Admonition type="note">
  `NFCCardManager` ships with the 3.x `android` artifact. That module is not
  published for SDK 4.0.0, so on current versions you implement
  `im.status.keycard.io.CardChannel` over your own `IsoDep` connection — the
  listener pattern below is still the shape to aim for.
</Admonition>
