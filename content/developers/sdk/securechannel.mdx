...
Only 10 clients can be permanently paired at once, but it is possible to unpair previously paired clients. Most importantly, an ephemeral pairing slot is available for one-time interactions with the card.
