...
Both the regular PIN and the duress PIN are accepted by this command and nothing in the response reveals which one was used. What changes is the key material the rest of the session operates on: authenticating with the duress PIN switches the active chain code, so the same BIP-32 paths deterministically yield a different set of keys and addresses. The session in progress is the only signal a client has of which wallet view is active.

Since 4.0 the duress chain code also drives the LEE keytree, so a card in [LEE mode](/developers/apdu/exportlee) presents a different LEE key set under the duress PIN as well.
