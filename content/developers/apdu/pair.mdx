...
<Admonition type="caution">
  **Removed in 4.0.** Pairing is gone: the secure channel is now authenticated
  by the card factory certificate, so a client connects to a card it has never
  seen without any shared secret or pairing state to persist. Applet 3.x cards
  are still in the field and cannot be upgraded, so clients that must support
  them implement pairing as well — see
...
P2:
* 0x00: prefer persistent pairing slot, fallback to ephemeral
* 0x01: ephemeral pairing slot
* 0x02: persistent pairing slot
