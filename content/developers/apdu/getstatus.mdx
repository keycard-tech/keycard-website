...
Before 4.0, `P1 = 0x01` returned the current key path as a sequence of 32-bit numbers, empty when the master key was selected. There is no current key path anymore — every [SIGN](/developers/apdu/sign) and [EXPORT KEY](/developers/apdu/exportkey) carries its own path — so `P1 = 0x01` now returns 0x6A86. Clients that cached the path from this command should track it locally instead.
