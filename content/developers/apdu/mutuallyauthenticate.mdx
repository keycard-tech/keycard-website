...
<Admonition type="caution">
  **Removed in 4.0.** Authentication of both parties is now folded into the
  [OPEN SECURE CHANNEL](/developers/apdu/opensecurechannel) handshake: the card
  signs the key exchange transcript, and the derived keys themselves prove the
  client saw that exchange. No second command is needed. Applet 3.x cards are
  still in the field and cannot be upgraded.
</Admonition>
