...
It’s natively integrated with all [Status apps](https://status.app) and works on iOS/Android mobile phones and desktops with a USB smartcard reader. [Learn more](https://keycard.tech/keycard).
Keycard communicates via NFC and ISO7816 interfaces. Communication with the Keycard happens through a simple APDU interface, together with a secure channel guaranteeing confidentiality, authentication and integrity of all commands.
